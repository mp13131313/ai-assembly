...
**Current state (2026-05-29 — Athens COMPLETE):** All three Athens nights ran end-to-end and are published — 13 dossiers + 30 per-voice pages across Nights 1–3 (athens-2026 `394914b`). athens-2026 has **10 of 10 panel voices shipped + promoted + uniformly named** under "Voice of X" convention: Voice of Plato, Voice of Cleopatra, Voice of Fyodor Dostoevsky, Voice of Ibn Battuta, Voice of the Octopus, Voice of Hannah Arendt, Voice of Ada Lovelace, Voice of the Whanganui River (**v2 architectural restructure + 4-field gap-E closure + kawa-speaker-frame closure** — witness-translator stance via `mediation_stance == "transmission_witness"`), Voice of Scheherazade, Voice of Bob Marley (v2 Option-3 restructure). All 10 voice_temporal_stance.default fields shifted to **assembly-fiction** ("voice present at the assembly that gathers in Athens, observes panels, responds when consulted"). Length-cap card surgery applied (Dostoevsky/Hannah Arendt 350-750w; Octopus 350-500w prose-channel front-loaded). `council_config.json` fully wired with all 10 pipeline-built provocateur profiles. **Plus 13th persona Tim Leberecht (Assembly editor)** SHIPPED at `athens-2026/editor/tim_leberecht/`; runtime `EDITOR_CARD_SUBPATH` rename `claudia_pinchbeck` → `tim_leberecht` (31/31 runtime tests pass). **Pre-Athens voice-build work is COMPLETE.** Operator-side residuals (Marley D1/E1 readiness paragraphs + Whanganui parallel + post-Athens reader gate scheduling) tracked in `_workspace/planning/voices/`. Earlier Claudia Pinchbeck DRAFT card DEPRECATED on Tim ship.
