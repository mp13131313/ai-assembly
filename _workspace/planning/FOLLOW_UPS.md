...
- **Build-side touchpoint:** Pass 4b's `quality_criteria` field (or Pass 5's, depending on where FU#49K's 5+2 criteria actually emit — verify; archived `_workspace/archive/2026-04-27_current_state_snapshot/CURRENT_STATE.md` §5.23 describes them as Pass 5, while Card v2 spec lists `quality_criteria` under Artifact = Pass 4b) could carry "Voice Pipeline Step 2 artifact MUST emit `strain_markers[]` array" as a quality criterion, anticipating the runtime contract. Half-day to add as a quality_criterion entry; runs cleanly into voices 3–12 buildout. Defer for now since the runtime consumer doesn't exist yet — would be ceremonial.
