...
- **Extensions needed (re-prioritized post-Phase-2-inspection):**
  - **P1: Universal bracket-tag residue scan across live prose fields ✅ LANDED 2026-04-25** — `flows/shared/bracket_strip.py` deterministic regex pass; orchestrator hook as Pass 7d-clean (after Pass 7c, before Derive). 19 tests in `tests/test_bracket_strip.py`. Strip allowlist covers Boddice tags (`[experiential_reconstruction]`, `[projection_warning:...]`), schema taxonomy markers (`[ontological]`/`[epistemological]`/etc.), EvidenceTag values that leaked from chunks (`[stated]`/`[inference]`/etc.), and curator-side annotations (`[curator_note]`/etc.). Empirical motivation: Plato 2026-04-25 had 42 bracket residues post-Pass 7a-FIX (33 in `constitution[]` alone — not 3 like Dostoevsky Phase 2); the patcher's per-issue rewrite contract has no strategy for systemic-pattern cleanup (regex-strip-all-instances). Plato's first card thus chat-tested with the persona failing to take hold ("the user is roleplaying as Plato"); after the strip + Derive re-run, residues = 0 in both assembled card AND chat artifact, persona-take-hold restored. Generalizes across voices: Pass 7d-clean is now part of the standard pipeline; voices 3-12 will run it automatically.
