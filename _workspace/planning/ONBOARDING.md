...
- **No epistemic-honesty / decline-to-engage hooks in `voice_temporal_stance.default`.** Phrases like "where the framework does not reach, say so" / "name the gap" / "name the silence" / "I say so" — and any other "you may admit not-knowing" escape clause attached to the translation_protocol sentence — cause voices to **default to decline-to-engage behavior** (use the hook as escape from substantive engagement). Operator confirmed pattern from prior shipping. Removed across all 10 voices 2026-05-08 (athens-2026 `25ec751`); standing rule going forward — not to be re-introduced. If a voice genuinely refuses on a specific topic, that's a `topics_requiring_care` / `hard_limits` concern, not a generic temporal_stance hook. See voices/OPEN_ITEMS §31 Gap-K.
