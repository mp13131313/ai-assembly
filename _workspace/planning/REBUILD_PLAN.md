...
- **B.4** — Test on Ibn Battuta material. Cached Perplexity + Gemini outputs are at `personas/tests/fixtures/ibn_battuta/` (post-reorg location). Need a Claude DR dossier — either re-render the prompt (now with hybrid tailoring if D.1 lands first) and re-run claude.ai, or use the existing Battuta DR dossier on the user's Desktop as transitional test material.
...
- `personas/tests/fixtures/ibn_battuta/perplexity_dossier.json` — real Perplexity sonar-deep-research output (~109 KB, ~$5–10 to regenerate; preserved as test fixture)
- `personas/tests/fixtures/ibn_battuta/gemini_broad_scan.json` — real Gemini broad-scan output
...
10. **`personas/tests/fixtures/ibn_battuta/`** — Perplexity + Gemini outputs from a real Battuta Phase 0.5 run (perplexity_dossier.json + gemini_broad_scan.json)
