...
| **Card field generation patterns** | Static field specs | **49D Position-B corpus-faithful self-cross-examination preamble** in Pass 2 `hard_limits` + hedge discipline (LANDED 2026-04-27, retained); **family-of-forms in Pass 4b + 49A v1 generativity-permitting Pass 4b + anti-generic-register banned_mode in Pass 4a — REVERTED 2026-04-28** (commit `3feb2b2`, empirical chat-test texture loss vs 2026-04-25 baseline); **FU#49A v2 quality_criteria 3-dim field-tied scaffold** rebuilt 2026-04-29 (commit `bfb7ed3`) over a 15-test exemplar journey — only piece of the 04-28 stack to land cleanly | Card v2.1 §H + §J + FOLLOW_UPS.md FU#49A |
...
| **Chat artifact invalidation gap (FU#52)** | Latent silent-staleness window | **CLOSED 2026-04-29** (commit `e11b65b`): chat_system_prompt path added to 7a-fix downstream invalidation list | run_persona_pipeline.py:7a-fix |
| **bold_engagement_topics runtime drop (FU#57)** | Runtime field across Step 1+2+3 + chat | **NEW 2026-04-29** (commit `9e8df77`): dropped from runtime system prompts (card_assembly.py + voice_step1/2/3 closing prompts) + chat artifact (chat_prompt_builder strip set). Pre-loaded courage menu pulled reasoning toward predetermined topics; chat-test on Plato performed better stripped. Field still emitted by Pass 5 — kept in card for build-side audit value | runtime/flows/voice/card_assembly.py + chat_prompt_builder.py |
...
- **REVERTED 2026-04-28** (originally landed `5506c8d` 2026-04-26 evening as FU#49H/I/J/K/L; reverted to `2bfddaa` baseline after empirical signal showed framework-edge-marking-as-signature tic across artifacts):
...
- **Active patterns (refreshed 2026-04-29 state):**
  - **System prompt at commit `bfb7ed3` = revert-to-418d553 baseline + FU#49A v2 quality_criteria refinement.** All other FU#49A/J content + family-of-forms emission + cryofreeze tense discipline were REVERTED on commit `3feb2b2` (2026-04-28 evening) due to empirical chat-test texture loss vs. 2026-04-25 baseline. See FOLLOW_UPS.md FU#49A entry for the full revert + v2 redesign trail.
...
- **REVERTED 2026-04-28** (originally landed `5506c8d` as FU#49K; reverted to `2bfddaa` baseline): closing-sentence requirement on `unique_contribution` (framework-strain closer). Voice's `unique_contribution` returns to substantive native-contribution closing without strain-marking-as-signature.
- `max_tokens=16000`.
